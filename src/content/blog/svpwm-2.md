...
<aside>
🧭
**系列导航** ｜ [SVPWM凭啥能把直流母线多榨15%？（上篇）六支箭、八种状态与六个扇区](/article/81722) ｜ 本篇：算边界、算时间 ｜ [SVPWM凭啥能把直流母线多榨15%？（下篇）开关顺序、过调制与完整实现](https://app.notion.com/p/SVPWM-15-498db07dd2654cebb468b67d2401445c?pvs=21)
...
V0、V7 都是零矢量，因此不改变平均电压的方向和幅值，只占用剩余时间。上面三条约束意味着：**$\boldsymbol U_{ref}$ 必须是八个开关矢量的非负时间加权平均，也就是这些矢量的凸组合。**六个有效矢量的凸包恰好是正六边形；位于中心的两个零矢量不会把这个凸包向外扩张。
...
上篇已经通过投影得到参考电压在 g、h 轴上的电压分量 $U_g$、$U_h$，并定义 $U_r=U_g+U_h$。这里继续统一约定：**$\boldsymbol e_0$ 对应 g 轴，$\boldsymbol e_{60}$ 对应 h 轴**，因此：

$$
\boldsymbol U_{ref}=U_g\boldsymbol e_0+U_h\boldsymbol e_{60}
$$

这一步不再求全局角度 $\theta$，也不再计算正弦函数，而是直接把 **$U_g$、$U_h$、$U_r$ 投影电压换算成相邻有效矢量的作用时间**。
...
**查表法**用“取数组值”代替实时计算正弦。只存 $0^\circ$～$60^\circ$ 的表即可，$\sin(60^\circ-\gamma)$ 可反向索引同一张表：256 点 Q15 表约占 512 B，float32 表约占 1 KB。优点是速度稳定；代价是占用 Flash、存在量化误差，线性插值虽能提高精度，却会增加乘加运算。还要注意：**正弦查表只省掉** $\sin$**，若局部角** $\gamma$ **尚未得到，**$\operatorname{atan2}$ **或角度查表仍然需要。**

下面给一个量级估算。统一假设：**100 MHz Cortex-M4F、float32、-O2 优化、无 CORDIC/三角函数硬件加速，从** $U_\alpha,U_\beta$ **计算扇区及两支非零矢量时间**。不同数学库、编译器和 Flash 等待周期会使结果上下浮动约 2～5 倍，最终仍应以目标芯片实测为准。
...
<aside>
⚙️
**换算成 20 kHz PWM 的时间预算**：$T_s=50\,\mu s$。仅“判区＋计算两支非零矢量时间”这一项，直接三角函数约占周期的 10%～40%；已知 $\gamma$ 的查表法约占 0.6%～2%；矩阵法约占 0.3%～1%。若查表法仍需软件计算 $\operatorname{atan2}$，占用会回升到约 5%～18%。所以变频器工程实现都选**矩阵法**。
...
下篇已整理为：[SVPWM凭啥能把直流母线多榨15%？（下篇）开关顺序、过调制与完整实现](https://app.notion.com/p/SVPWM-15-498db07dd2654cebb468b67d2401445c?pvs=21)
